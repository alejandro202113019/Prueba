    ...
    <form action="index.php?entity=hallazgo&action=edit&id=<?= $hallazgo['id'] ?>" method="POST">
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <!-- CAMPO ACTUALIZADO: Sede -->
                <div class="form-group">
                    <label for="sede_id">Sede 🏢</label>
                    <select class="form-control <?= $hallazgo['sede_id'] ? 'border-success' : '' ?>" id="sede_id" name="sede_id">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Tooltip para procesos múltiples
    procesosSelect.title = 'Mantén presionada la tecla Ctrl (o Cmd en Mac) para seleccionar múltiples procesos';
    
    // Resaltar sede cuando se cambia
    ...
    form.addEventListener('submit', function(e) {
        const sedeOriginal = '<?= $hallazgo['sede_id'] ?? '' ?>';
        const sedeNueva = sedeSelect.value;
        
        if (sedeOriginal !== sedeNueva) {
            const mensaje = sedeNueva ? 
                'Has cambiado la sede asignada. ¿Confirmas guardar este cambio?' :
                'Vas a quitar la sede asignada. ¿Confirmas este cambio?';
